...
export function AuthShell({ badge, title, description, footerText, footerLink, children }: AuthShellProps) {
  return (
    <main className="relative min-h-screen overflow-hidden bg-[radial-gradient(ellipse_at_top_left,_#ebeafc_0%,_#f4f5f7_38%,_#eef0fd_100%)] px-4 py-8 text-[var(--color-text-primary)] sm:px-6 lg:px-8">
      ...
      <div className="pointer-events-none absolute inset-0 opacity-[0.35] [background-image:radial-gradient(rgba(143,151,176,0.28)_1px,transparent_1px)] [background-size:18px_18px]" />
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-6xl flex-col">
        <header className="flex items-center justify-center py-3 sm:justify-start">
          <Link href="/" className="flex items-center gap-3 text-[15px] font-bold tracking-[-0.02em]">
            <span className="flex h-11 w-11 items-center justify-center rounded-[12px] bg-[linear-gradient(135deg,#6b5fef_0%,#5b4fe8_100%)] text-white shadow-[var(--shadow-primary)]">
              <Camera size={22} strokeWidth={2.2} />
            </span>
            <span className="flex flex-col leading-none">
              <span className="text-[18px] font-bold text-[var(--color-text-primary)]">GrabPic</span>
              <span className="text-[11px] font-medium tracking-[0.12em] text-[var(--color-text-tertiary)] uppercase">
        ...
        <section className="grid flex-1 items-center gap-10 py-6 lg:grid-cols-[0.95fr_1.05fr] lg:py-10">
          <div className="mx-auto w-full max-w-xl space-y-8 text-center lg:text-left">
            <div className="space-y-4">
              <Badge className="w-fit border border-white/70 bg-white/80 px-3 py-1.5 text-[11px] tracking-[0.12em] text-[var(--color-primary)] shadow-[0_10px_30px_rgba(91,79,232,0.08)] backdrop-blur-xl">
                {badge}
              </Badge>
              <div className="space-y-4">
                <h1 className="text-4xl font-extrabold tracking-[-0.04em] text-[var(--color-text-primary)] sm:text-5xl lg:text-6xl">
                  {title}
                </h1>
                <p className="mx-auto max-w-lg text-base leading-7 text-[var(--color-text-secondary)] lg:mx-0 lg:text-lg">
                  {description}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-1 xl:grid-cols-3">
              {highlights.map((item) => {
                ...
                return (
                  ...
                  >
                    <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-[12px] bg-[var(--color-primary-subtle)] text-[var(--color-primary)]">
          ...
          <div className="mx-auto w-full max-w-[520px]">
            ...
            <p className="mt-6 text-center text-sm text-[var(--color-text-secondary)]">
              {footerText}{" "}
              <a href={footerLink.href} className="font-semibold text-[var(--color-primary)] hover:underline">
                {footerLink.label}
              </a>
            </p>
          </div>
